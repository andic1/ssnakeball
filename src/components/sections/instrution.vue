<template>
    <div class="instrution _fullscreen" v-show="instrution.if_visible.value">
        ...
        <div class="instrution_text _font_2">
            <p>· 红色大球：危险物，蛇球不要碰，尽量绕开。</p>
            <p>· 绿色大球：奖励球，蛇球主动吃掉可以加分/回血。</p>
            <p>· 彩色圆环：增益道具，穿过后会触发特殊效果（速度等会变化）。</p>
            <p>· 随着时间推进，蛇球的速度和拖尾长度会自动刷新，请根据当前状态选择“吃”还是“躲”。</p>
        </div>
...
.instrution_text {
    position: absolute;
    bottom: calc(var(--scale) * 4rem);
    left: 50%;
    transform: translateX(-50%);
    flex-direction: column;
    gap: calc(var(--scale) * 1.2rem);
    text-align: left;
    color: var(--color_front);
    width: min(92vw, calc(var(--scale) * 120rem));
    padding: calc(var(--scale) * 2.2rem) calc(var(--scale) * 2.6rem);
    border-radius: calc(var(--scale) * 2rem);
    border: calc(var(--scale) * 0.18rem) solid color-mix(in srgb, var(--color_front) 20%, transparent);
    background: color-mix(in srgb, var(--color_back) 92%, transparent);
    box-shadow: 0 calc(var(--scale) * 1.2rem) calc(var(--scale) * 4rem)
        color-mix(in srgb, var(--color_front) 18%, transparent);
}
